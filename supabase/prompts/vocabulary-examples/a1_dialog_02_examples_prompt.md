...
- ที่ไหน (tîi-nǎi) = where  [key: where]  ·  question_word
- ไป (bpai) = go  [key: go]  ·  verb  ·  Also marks movement away or future intention in simple patterns.
- ดื่ม (dʉ̀ʉm) = drink  [key: drink]  ·  verb
- กาแฟ (gaa-faae) = coffee  [key: coffee]  ·  noun
- ได้ (dâai) = can  [key: can]  ·  verb  ·  Used for ability possibility permission and some completed actions.
- ด้วยกัน (dûai-gan) = together  [key: together]  ·  adverb
