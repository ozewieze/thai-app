# Illustration Prompt — a1_dialog_04 / Slide 2
...
- Slide Specification: `slide-specs/a1_dialog_04_slide_specs.md` → Slide 2

## Verplicht mee te sturen referentieafbeeldingen (niet optioneel)

1. `public/hero-image.png` — stijl-/paletreferentie
2. Face lock-referenties van Mali en Narin (front + 3/4 + profiel). Beide personages blijven naar elkaar gekeerd aan hetzelfde tafeltje — driekwart blijft de belangrijkste hoek, stuur alle drie mee.
...
4. De goedgekeurde afbeelding van Slide 1 (`slide-00.png`) van dezelfde dialoog — continuïteitsreferentie én **background lock**-referentie (zelfde scène, enkele seconden later; de achtergrond moet pixelconsistent blijven, niet slechts vergelijkbaar).
...
Moment: Still nothing on the table — Narin asks which snack Mali wants;
she names cake without hesitation. The choice exists only in the
conversation, not yet in front of them. Narin's expression turns curious,
one hand lightly spread in a "go ahead, tell me" gesture; Mali answers
with a small spontaneous brightness — perhaps her hands coming together
briefly at the thought of cake, rather than pointing at anything. Same
seated positions as the previous slide, moments later; Mali sits slightly
more forward with anticipation rather than mirroring Narin's earlier
posture. Narin curious and attentive; Mali delighted and certain. Light
and decisive — the natural specificity beat of ordering.
...
2. Afbeelding downloaden en hernoemen: `slide-01.png` (zero-padded, `slide_index = 1`).
3. Zetten in `illustration-staging/a1-dialog-04/slide-01.png`.
4. Uploaden via `scripts/upload-slides.mjs` (Stap 7-8) → pad `illustrations/dialogs/a1/dialog-04/slides/slide-01.png`, werkt `dialog_slides.image_url` bij voor `slide_index = 1`.
