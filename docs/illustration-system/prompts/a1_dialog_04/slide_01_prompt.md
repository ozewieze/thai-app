# Illustration Prompt — a1_dialog_04 / Slide 1
...
- Scene Bible: `scene-bibles/a1_dialog_04_scene_bible.md`
- Slide Specification: `slide-specs/a1_dialog_04_slide_specs.md` → Slide 1

## Verplicht mee te sturen referentieafbeeldingen (niet optioneel)

1. `public/hero-image.png` — stijl-/paletreferentie
2. Face lock-referenties van Mali en Narin (front + 3/4 + profiel). Dit Moment is opnieuw een net-gezeten moment aan hetzelfde cafétafeltje als Dialog 3, met beide personages licht naar elkaar gekeerd — driekwart is hier de belangrijkste hoek, stuur alle drie mee.
...
4. n.v.t. — dit is de eerste slide van deze dialoog, geen vorige slide binnen `a1_dialog_04` om als continuïteitsreferentie te gebruiken. (Optioneel, niet verplicht: omdat dialoog 4 direct aansluit op dialoog 3, aan dezelfde tafel, kan de laatste goedgekeurde slide van `a1_dialog_03` meegestuurd worden als extra locatie-/sfeercontinuïteit.)

**Let op:** deze slide legt de vaste achtergrond (camerahoek, framing, meubels, lichtinval) vast waar Slide 2 en Slide 3 van deze dialoog pixelconsistent op moeten aansluiten (zie Background Lock Rule in `01_master_style_system.md`, meegenomen in de prompts van Slide 2 en 3). Er staat op tafel nog niets — geen drankjes, geen snacks, geen menu — dit is een puur gesproken aanbod.
...
corner of a coffee shop, seated at the same small café table as Dialog 3,
...
Clean and uncluttered overall. Color palette: warm off-whites, muted warm
neutrals, soft sand tones, restrained saturation. Nothing has been served
yet — no cups, plates, or food visible on the table.

Moment: Nothing has been ordered or served yet — this is purely a spoken
offer. Narin asks Mali if she'd like a snack; she accepts warmly. Narin
sits back a little and raises his eyebrows in a light, inviting way, palms
resting open on the table rather than pointing at anything; Mali responds
with a quick, pleased smile and a small settling shift in her seat. Both
still seated as before; Narin's posture opens up slightly (shoulders
relaxed, hands open) rather than leaning in. Narin warm and inviting; Mali
brightening, pleasantly surprised by the offer. Warm, easy continuation —
a natural, unforced transition to snacks.
...
3. Zetten in `illustration-staging/a1-dialog-04/slide-00.png`.
4. Uploaden via `scripts/upload-slides.mjs` (Stap 7-8) → pad `illustrations/dialogs/a1/dialog-04/slides/slide-00.png`, werkt `dialog_slides.image_url` bij voor `slide_index = 0`.
