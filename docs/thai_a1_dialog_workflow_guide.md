...
Het verwijderen van zo'n link volstaat: `trg_lesson_vocabulary_revert_on_delete` zet `vocabulary_status` daarna automatisch terug op `new` met `first_lesson_id = null`, mits het woord nergens anders meer in `lesson_vocabulary` voorkomt. Dezelfde `AFTER DELETE`-triggers bestaan voor grammatica, patterns en phrases.
